 ...
 * DGFT_IRM_MESSAGE_DETAIL) in batches of dgft.scheduler.irm-push.batch-size
 * (default 20) records per transaction, and updates DGFT_IRM_MASTER per the
 * flow doc:
 *   FLAG            : P
 ...
 *
 * Each call to pushNextBatchToDgft() stages exactly one batch in its own
 * transaction. Draining the full backlog (calling this repeatedly until
 * no fresh records remain) is the Scheduler's responsibility - see
 * DgftIrmPushScheduler - so that batches genuinely commit independently
 * instead of being folded into a single long-running transaction.
 */
@Service
...
public class DgftIrmPushServiceImpl implements DgftIrmPushService {
    ...
    @Override
    @Transactional
    public boolean pushNextBatchToDgft() {
        ...
        if (!success) {
            ...
            log.error("IRM staging push failed. TxId={}, batchSize={}",
                    request.getUniqueTxId(), request.getIrmList().size());
            // The batch failed, but it WAS an attempted batch - still true so
            // the scheduler doesn't misreport the run as fully drained.
            // (The failed records keep DGFT_STATUS = "Awaiting request
            // initiated" untouched below, so they'll be retried next run.)
            return true;
        }
        ...
        log.info("IRM batch staged for DGFT push. TxId={}, Records={}",
